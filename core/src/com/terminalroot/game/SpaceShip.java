...
public class SpaceShip extends ApplicationAdapter {
	...
	public void create () {
		...
		xMissile = posX;
		yMissile = posY;
		attack = false; // tiros na barra de SPACE
	...
	private void moveNave() {
		if (Gdx.input.isKeyPressed(Input.Keys.RIGHT)){ // COMANDO PARA APERTAR TECLA PARA DIREITA , JUNTO COM O IMPORT Gdx;
			if (posX < Gdx.graphics.getWidth() - nave.getWidth()){
			posX += velocity;
			}
		}
		if (Gdx.input.isKeyPressed(Input.Keys.LEFT)){ // COMANDO PARA APERTAR TECLA PARA ESQUERDA , JUNTO COM O IMPORT Gdx;
			if (posX > 0) {
				posX -= velocity;
			}
		}
		if (Gdx.input.isKeyPressed(Input.Keys.UP)){ // COMANDO PARA APERTAR TECLA PARA CIMA , JUNTO COM O IMPORT Gdx;
			if (posY < Gdx.graphics.getHeight() - nave.getHeight()) {
				posY += velocity;
			}
			}
		if (Gdx.input.isKeyPressed(Input.Keys.DOWN)){ // COMANDO PARA APERTAR TECLA PARA BAIXO , JUNTO COM O IMPORT Gdx;
			if (posY > 0) {
				posY -= velocity;
			}
		}
	...
	private void moveMissile() {
		...
		if (attack) {
			if (xMissile < Gdx.graphics.getWidth() ) { // para o tiro voltar para a tela
				xMissile += 40;
			}else{
